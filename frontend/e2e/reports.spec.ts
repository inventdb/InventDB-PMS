...
const galleryItem = (page: import("@playwright/test").Page, name: string) =>
  page.locator(".report-item").filter({ hasText: name });
...
test.describe("Reports gallery", () => {
  ...
  test("lists saved reports grouped by category", async ({ page }) => {
    await expect(page.locator(".count-pill")).toHaveText("3 reports");
    await expect(page.locator(".report-gallery .nav-section")).toHaveText([
      "Financial",
      "Leasing",
    ]);
    await expect(page.locator(".report-item")).toHaveCount(3);
    await expect(galleryItem(page, "Owner Statement")).toContainText(
      "Monthly income and expense summary per owner."
    );
  });

  test("opens the first report automatically", async ({ page }) => {
    await expect(galleryItem(page, "Owner Statement")).toHaveClass(/active/);
    await expect(page.getByRole("heading", { name: "Owner Statement", level: 3 })).toBeVisible();
  });

  test("filters the gallery", async ({ page }) => {
    await page.getByPlaceholder("Find a report…").fill("rent");
    await expect(page.locator(".report-item")).toHaveCount(1);
    await expect(galleryItem(page, "Rent Roll")).toBeVisible();
  });

  test("says so when nothing matches", async ({ page }) => {
    await page.getByPlaceholder("Find a report…").fill("zzzz");
  ...
  test("switches the stage when another report is picked", async ({ page }) => {
    await galleryItem(page, "Rent Roll").click();
    await expect(galleryItem(page, "Rent Roll")).toHaveClass(/active/);
    await expect(page.getByRole("heading", { name: "Rent Roll", level: 3 })).toBeVisible();
    // Scope to the stage: the same description also renders in the gallery
    // card, so an unscoped text match is ambiguous.
    await expect(
      page.locator(".report-stage").getByText("Every active lease with contract and market rent.")
    ).toBeVisible();
  });
...
test.describe("Rendering a report", () => {
  ...
  test("waits for input when a required parameter cannot be seeded", async ({ page }) => {
    await page.goto("/reports");
    await galleryItem(page, "Region Audit").click();
